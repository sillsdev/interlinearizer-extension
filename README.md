...
- `studioVersion` and `studioRunId`: The Paratext 10 build to attach to this release — its version (e.g. 0.4.0-alpha.1) and the ID of the `paranext/paratext-10-studio` Package workflow run that built it (e.g. 35758007497, from the run's URL). That repo is private and needs granted access. Only runs dispatched with **Upload the release assets to the configured S3 bucket** checked are usable: the workflow downloads the Windows, macOS, and Linux installers from that bucket, attaches them to the release, and substitutes the version into the release body. Pick the newest such run, so its paranext-core revision is as close as possible to the one this workflow builds against (recorded in its job summary). To read a run's version and S3 locations: `gh run view <run ID> -R paranext/paratext-10-studio --log | grep -o 'https://paratext-10-studio-releases[^]]*'`.
