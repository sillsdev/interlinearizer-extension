...
`interlinearizer-extension`, `main` @ `7827c72`.
...
Two origin vocabularies run through Table A, on different axes: `NavOrigin` on the reference write,
and the five-valued `FocusOrigin` on the focus write. Table B carries the couplings between rows —
where one entry point disables or cancels another mid-flight.
...
1. **Rows 4 and 5 disagree about the strip on purpose, and say so in a type.** Both are `internal`
   navigations as far as the _list_ is concerned. The strip asks a different question — did _I_ make
   this move? — and answers it by reading `FocusOrigin`, so a `list` write fades it and a `strip`
   write glides. Two meanings of "internal" coexist, one per subsystem, as two named vocabularies
   rather than one word doing double duty.
...
2. **Row 1 can leave the strip completely still.** The verse rule early-returns when the focused
   ...
   segment_ fades and rebuilds the list while the strip does not move at all. Intentional, and the
   rule and its reason sit together in `FocusStore.tsx` so it does not read as a bug.
...
[^5]:
    ...
    timer will never run. A cleanup drops the timer before the superseding run can see it, which
    would leave a phrase click on a half-faded box stranding the strip at opacity 0. See asymmetry 8.
...
[^9]:
    ...
    so it outranks both reseeds by construction rather than by any ordering between effects.
