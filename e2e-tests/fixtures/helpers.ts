 ...
 *
 * Cancel is preferred over Escape because a modal that died mid-submission suppresses Escape on
 * purpose. Modals can chain, so cancel in a bounded loop until none remains.
 *
 * @param page The Playwright `Page` for the Platform.Bible renderer window.
 * @returns Resolves once no modal remains in the iframe (a no-op on the common clean path).
 */
