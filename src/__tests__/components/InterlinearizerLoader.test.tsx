...
describe('InterlinearizerLoader', () => {
  ...
  it('leaves the loading text blank while its localize key is unresolved', async () => {
    // `localizedStringsAsKeys` stands in for PAPI's pre-resolution state, where each key maps to
  ...
  it('shows the loading text once its localize key resolves', async () => {
    jest.mocked(useLocalizedStrings).mockImplementation((keys) => {
      const [strings, isLoading] = localizedStringsAsKeys(keys);
      return [{ ...strings, '%interlinearizer_loading%': 'Loading…' }, isLoading];
    });
    mockBookData({ book: undefined, isLoading: true });
    await act(async () => {
      renderLoader();
