...
describe('getArcStrokeProps', () => {
  ...
  const highlighted = { stroke: 'var(--foreground)', strokeOpacity: 0.75, strokeWidth: 2 };
  const destructive = {
  ...
  it('uses foreground at 75% opacity for the focused phrase arc in view mode', () => {
    expect(getArcStrokeProps({ kind: 'view' }, 'p1', undefined, 'p1')).toEqual(highlighted);
  });

  it('uses foreground at 75% opacity for the focused phrase even when it is also hovered', () => {
    expect(getArcStrokeProps({ kind: 'view' }, 'p1', 'p1', 'p1')).toEqual(highlighted);
  });

  it('uses foreground at 75% opacity for the edited phrase arc in edit mode regardless of hover', () => {
    expect(
      getArcStrokeProps(
