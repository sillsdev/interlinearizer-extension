...
export function TokenLinkIcon({
  ...
  // segment (a within-segment link) or this slot is the adjacent edge of the focused segment (a
  // cross-segment link that pulls the edge token across and moves the boundary).
  const isActive =
    phraseMode.kind === 'view' &&
    focusedSideIsPrev !== undefined &&
    (isSameSegmentAsFocus || isAdjacentEdgeOfFocus);
  const linkDisabled = isUnlinkMode || isEditMode || !isActive;
  // Show a tooltip only when inactive because the slot is a non-edge cross-segment slot (not when
