 ...
 * when the caller's store is the host's. An arrow key reports on each press.
 */
export default function usePanelResize(
  ...
  const onKeyDown = useCallback(
    (event: ReactKeyboardEvent) => {
      // Reading the visual direction rather than the writing direction: the key that moves the
      // handle toward the start edge widens the panel, whichever edge the interface language puts
      // that on.
