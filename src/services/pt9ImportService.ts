...
import { convertPt9Project, Pt9ImportReport, Pt9ProjectData } from '../converters/pt9';
 ...
 * first import and sync: fetches the raw files through the read-only Pt9Interlinear
 * projectInterface, parses them, rebuilds the text layer for every book they reference from the
 * project's USJ, converts, and persists the outcome as the source's single frozen import project -
 * created on first run, replaced wholesale on later runs. The stored name and description are the
 * fixed localized values, resolved at import time.
 *
 ...
 * @throws If a file fails to parse, the conversion rejects the input, or persistence fails. Nothing
 *   has been written unless persistence itself failed.
 */
export async function importPt9Project(
  ...
  const files = await pt9Pdp.getPt9InterlinearFiles();
  // Path-sorted for a deterministic parse order; gloss-language order follows it.
  ...
  const data: Pt9ProjectData = { interlinear: [] };
  paths.forEach((path) => {
    const { text } = files[path];
    if (path === 'Lexicon.xml') data.lexicon = new LexiconXmlParser().parse(text);
    else if (path === 'WordAnalyses.xml')
      data.wordAnalyses = new WordAnalysesXmlParser().parse(text);
    else if (path === 'InterlinearSetup.xml')
      data.setups = new InterlinearSetupXmlParser().parse(text);
    // The projectInterface serves only the four known patterns, so every other path is a
    // per-language interlinear file.
    else data.interlinear.push(new InterlinearXmlParser().parse(text));
  });

  const bookIds = [...new Set(data.interlinear.map((file) => file.BookId))];
  ...
  const fileHashes = Object.fromEntries(paths.map((path) => [path, files[path].sha256]));
