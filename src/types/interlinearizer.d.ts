 ...
 * whose id space its ids belong to. Where the Lexicon extension does not yet surface a referenced
 * type or provide a lookup method (see the per-ref "Current Lexicon gap" notes below), this model
 * is the standard and the extension is expected to add what's missing. Summary of gaps:
 *
...
declare module 'interlinearizer' {
  ...
  /**
   * Labels an **authority**: the id space a lexicon's ids belong to. An authority is who minted an
   * id, not who can resolve it; two lexicons that mint ids in the same form are still separate
   * authorities, and an id means nothing outside the one that minted it.
   *
  ...
  /**
   * What every lexicon reference carries besides its id. Accept it where only the authority
   * matters.
   *
   * A ref is **foreign** when no registered resolver declares its authority. A foreign ref is never
   * resolved, never passed to a resolver, and never dropped: it survives reads, writes, and saves
   * unchanged, and rendering falls back to the stored free-form gloss. A ref whose authority _is_
   * declared goes to that resolver even when its `projectId` names a project that is not open; that
   ...
   * gloss text can be surfaced and refreshed automatically if the lexicon is edited. Both may be
   * present simultaneously; when they are, `gloss` takes precedence for rendering (the local
