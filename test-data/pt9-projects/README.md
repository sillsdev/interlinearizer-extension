...
with no cluster of their own, so an analysis approved elsewhere has somewhere to be suggested, and
one form approved four times over, so its pool bucket holds competing payloads.

Cluster `Range` values are character offsets into the verse text, so changing a verse means
recomputing every range at or after the change, in every gloss language for that book.
